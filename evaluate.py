...
class Evaluator():
    def __init__(self, eval_tasks, align_func, is_save_all_tables=False, clean_data=True) -> None:
        ...
        self.is_save_all_tables = is_save_all_tables
        warning(f"Saving the result is: {self.is_save_all_tables}")
...
class evaluateMultiCheckpoints:
    ...
    def evaluateOneCheckpoint(self, ckpt):
        ...
        evaluator = Evaluator(eval_tasks=self.tasks, align_func=infer.inference, is_save_all_tables=True)
    ...
    def init_eval_dataset(self):
        ...
        for eval_task in self.eval_tasks:
            if eval_task in HUGGINGFACE_DATASETS:
                ...
                self.dataset[eval_task] = pd.read_csv('paws_qqp/output/dev_and_test.tsv', sep='\t')
            elif eval_task == 'beir':
                print("beir load by itself")
                self.dataset[eval_task] = "BEIR Benchmark"
            elif eval_task in PICKLE_DATASETS:
